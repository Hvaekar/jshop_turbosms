...
class plgjshoppingorderSms_status_change extends JPlugin
{
    ...
    public function onAfterChangeOrderStatusAdmin($order_id, $status, $status_id, $notify, $comments, $include_comment, $view_order, $prev_status)
    {
        $order = \JSFactory::getTable('order');
        $order->load($order_id);
        $vendorinfo = $order->getVendorInfo();

        if (!$this->_params->active || !$this->_params->turbosms_login || !$this->_params->turbosms_password || !$order->mobil_phone || (!$this->_params->always && !$notify) || ($status == $prev_status)) return;

        $message = $this->getMessage($status, $order, $comments, $include_comment, $vendorinfo);
        if (!$message) return;

        $this->sendSMS($order->mobil_phone, $message);
    }

    public function onAfterChangeOrderStatus($order_id, $status, $sendmessage, $prev_status)
    {
        $order = \JSFactory::getTable('order');
        $order->load($order_id);
        $vendorinfo = $order->getVendorInfo();

        if (!$this->_params->active || !$this->_params->turbosms_login || !$this->_params->turbosms_password || !$order->mobil_phone || ($status == $prev_status)) return;

        $message = $this->getMessage($status, $order, '', 0, $vendorinfo);
        if (!$message) return;

        $this->sendSMS($order->mobil_phone, $message);
    }

    private function sendSMS($mobil_phone, $message)
    {
    ...
    private function getMessage($status, $order, $comments, $include_comment, $vendorinfo)
    {
